...
public class Rikishi {
     ...
     * @param position          初期位置（x座標はcenter,y座標はbottom）
     * @param tapRect           tapイベントを受け取るviewの領域
     * @param oppositeDirection trueならx軸逆向きに進む
     */
    public Rikishi(int playerId, Bitmap bitmap, PointF position, RectF tapRect,
                   boolean oppositeDirection,final PointF JUMP) {
     ...
     *
     * @param force      衝突後の速さ
     * @param collisionX 衝突した座標
     */
    public void onCollision(float force, float collisionX) {
        // 位置補正
        position.offsetTo(
                ((collisionX < position.centerX()) ? collisionX + 4.0f
                        : (collisionX - width - 4.0f)), position.top);
        // 速度交換
        velocity.x = force;
    }
